...
_detected = st.session_state.get("detected_city", "")
header_html = "<div class='weather-grid'>"
for city in city_names:
    _marker = " 📍" if city == _detected else ""
    header_html += f"<div class='weather-city-header'>{city}{_marker}</div>"
...
if send_clicked:
    ...
    if not msg and img_b64 is None:
        ...
        # Do NOT rotate widget_counter here — image and textarea stay
        # until user explicitly clicks Remove Image or Clear.
        st.session_state.pending_image    = None
        st.session_state.pending_img_name = None
        st.session_state.pending_img_b64  = None
        st.rerun()
