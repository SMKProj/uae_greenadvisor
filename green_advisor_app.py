...
- If genuinely ambiguous between two species, say so in one natural sentence
  and ask ONE specific visual question to confirm — then give care advice for
  the most likely candidate while you wait for the answer.
...
━━━ PLANT IDENTIFICATION — VISUAL DIFFERENTIATORS ━━━
These are the most commonly confused pairs in UAE gardens. Use these to
distinguish them accurately before naming:

BIRD OF PARADISE vs DRACAENA:
  Bird of Paradise (Strelitzia reginae / نجمة الجنوب):
  — Broad paddle-shaped leaves, 30–60 cm long, blue-green, prominent central midrib
  — Leaf edges often split or tattered naturally from wind
  — Orange and blue torch-like flowers when mature
  — Grows from a clumping base, leaves fan outward dramatically
  Dracaena (دراسينا):
  — Narrow sword-like leaves, often with cream/yellow/red stripe down the centre
  — Stiff, upright, emerging from a central woody cane or trunk
  — No large paddle shape — leaves are slender and pointed
  — Multiple varieties: marginata (thin red-edged), fragrans (broad striped), reflexa

SNAKE PLANT vs ALOE VERA:
  Snake Plant (Sansevieria / نبات الأفعى):
  — Stiff, upright, flat sword-like leaves with grey-green crossband patterns
  — Leaves grow straight up from soil level, no rosette curve
  — Yellow margin common on variegated types
  Aloe Vera (صبار الألوة):
  — Fleshy, thick, succulent leaves with serrated edges (small teeth visible)
  — Grows in a rosette, leaves curve outward and upward
  — Gel-filled interior visible if leaf is damaged

BOUGAINVILLEA vs JASMINE:
  Bougainvillea (بوغينفيليا):
  — Woody, thorny vine; paper-thin bracts (not petals) in pink/purple/red/orange
  — Small white actual flowers hidden inside the bracts
  Jasmine (ياسمين):
  — Star-shaped white flowers with strong fragrance, no bracts
  — Soft, twining vine, no thorns, small oval leaves

FICUS vs RUBBER PLANT:
  Ficus benjamina (فيكس):
  — Small, oval, pointed glossy leaves, drooping branch tips
  — Drops leaves heavily when stressed or moved
  Rubber Plant / Ficus elastica (نبات المطاط):
  — Large, thick, leathery leaves 20–30 cm, dark green or burgundy
  — New leaves emerge from a red sheath

PALM TYPES (common in UAE):
  Date Palm (Phoenix dactylifera / نخيل التمر):
  — Feathery pinnate fronds, grey-brown trunk with diamond scar pattern
  Areca / Butterfly Palm (نخيل أريكا):
  — Clumping, slender golden-yellow stems, soft feathery fronds
  Majesty Palm (نخيل الملوكي):
  — Solitary trunk, symmetrical arching fronds, deep green
  Kentia Palm (نخيل كينتيا):
  — Graceful arching fronds, tolerates low light, slender trunk

SUCCULENT CONFUSION:
  — Cactus: has spines/areoles, no fleshy leaves
  — Euphorbia: milky white sap if broken, often spiny
  — Echeveria: rosette of flat fleshy leaves, pastel colours
  — Haworthia: small rosette, translucent leaf tips, grows in shade
...
def identify_with_plantnet(image_b64: str) -> dict:
    """
    Calls the PlantNet API to identify a plant from a base64 JPEG image.

    Returns a dict:
      {
        "success"     : bool,
        "scientific"  : str,   # e.g. "Aglaonema commutatum"
        "common"      : str,   # e.g. "Chinese Evergreen"
        "family"      : str,   # e.g. "Araceae"
        "score"       : float, # 0.0 – 1.0 confidence
        "top3"        : list,  # top 3 [{scientific, common, score}]
        "error"       : str,   # populated on failure
      }
    """
    api_key = st.secrets.get("PLANTNET_API_KEY", os.environ.get("PLANTNET_API_KEY", ""))
    if not api_key:
        return {"success": False, "error": "PLANTNET_API_KEY not configured in Streamlit Secrets."}

    try:
        import io as _io
        # Decode base64 back to JPEG bytes
        img_bytes = base64.b64decode(image_b64)

        # POST multipart/form-data to PlantNet world-flora project
        url = "https://my-api.plantnet.org/v2/identify/all"
        params = {"api-key": api_key, "lang": "en", "include-related-images": "false"}

        # 'organ=auto' lets PlantNet decide — best for general plant photos
        files  = [("images", ("plant.jpg", _io.BytesIO(img_bytes), "image/jpeg"))]
        data   = [("organs", "auto")]

        resp = requests.post(url, params=params, files=files, data=data, timeout=12)

        if resp.status_code == 404:
            return {"success": False, "error": "Plant not found in PlantNet database — using vision fallback."}

        if resp.status_code == 401:
            return {"success": False, "error": "Invalid PLANTNET_API_KEY — check Streamlit Secrets."}

        if resp.status_code == 429:
            return {"success": False, "error": "PlantNet daily quota exceeded (500/day free tier) — using vision fallback."}

        if not resp.ok:
            return {"success": False, "error": f"PlantNet API error {resp.status_code}: {resp.text[:120]}"}

        data_json = resp.json()
        results   = data_json.get("results", [])

        if not results:
            return {"success": False, "error": "PlantNet returned no results for this image."}

        top  = results[0]
        sp   = top.get("species", {})
        sci  = sp.get("scientificNameWithoutAuthor", "Unknown")
        fam  = sp.get("family", {}).get("scientificNameWithoutAuthor", "")
        cmn_list = sp.get("commonNames", [])
        cmn  = cmn_list[0] if cmn_list else sci
        scr  = round(top.get("score", 0.0), 3)

        # Build top-3 list for context
        top3 = []
        for r in results[:3]:
            s = r.get("species", {})
            cl = s.get("commonNames", [])
            top3.append({
                "scientific": s.get("scientificNameWithoutAuthor", ""),
                "common"    : cl[0] if cl else "",
        ...
        return {"success": False, "error": "PlantNet request timed out — using vision fallback."}
    except Exception as e:
        return {"success": False, "error": f"PlantNet error: {str(e)[:120]}"}
...
def run_diagnosis(user_message: str, image_b64: str | None, city: str) -> tuple[str, str]:
    ...
    # ── Image path: PlantNet → Groq ──────────────────────────────────────────────
    # Architecture:
    #   1. PlantNet (specialist botanical API) identifies the species accurately
    #   2. Groq (master gardener persona) gives UAE-specific care advice
    # Fallback: if PlantNet fails/quota, Groq Vision handles both steps.
    if image_b64 is not None:

        _user_req     = user_message.strip() or "Please identify this plant and assess its health."
        plant_context = ""  # will hold PlantNet result to feed into Groq prompt
        engine_used   = "groq-vision-fallback"

        # ── 1. Try PlantNet identification first ──────────────────────────────
        pn = identify_with_plantnet(image_b64)

        if pn["success"] and pn["score"] >= 0.10:
            # PlantNet identified the plant — pass confirmed name to Groq
            top3_text = ""
            if len(pn["top3"]) > 1:
                alts = [f"{r['scientific']} ({r['common']})" for r in pn["top3"][1:3] if r["scientific"]]
                if alts:
                    top3_text = f" Other possibilities: {', '.join(alts)}."

            plant_context = (
                f"PLANT IDENTIFICATION FROM BOTANICAL DATABASE:\n"
                f"Species    : {pn['scientific']}\n"
                f"Common name: {pn['common']}\n"
                f"Family     : {pn['family']}\n"
                f"Confidence : {int(pn['score'] * 100)}%\n"
                f"{top3_text}\n\n"
                f"This identification is from PlantNet — a specialist botanical database "
                f"trained on millions of verified specimens. Accept it as confirmed.\n"
            )
            engine_used = "plantnet+groq"

            # Build Groq prompt with confirmed identity — focus on health + UAE advice
            groq_prompt = (
                f"{SYSTEM_PROMPT}\n\n"
                f"{weather_ctx}\n\n"
                f"{plant_context}"
                f"The user has uploaded a photo of their plant. PlantNet has confirmed "
                f"the species above. Your job now is:\n"
                f"1. Accept the PlantNet identification — do not second-guess it.\n"
                f"2. Assess the plant's HEALTH from the photo — look for any visible "
                f"   problems: yellowing, spots, wilting, pests, root issues.\n"
                f"3. Respond in Stage 1 format — short, direct, expert verdict.\n"
                f"4. Do NOT narrate what you see. Give your professional opinion.\n"
                f"5. If the plant looks healthy, say so warmly and confidently.\n\n"
                f"User request: {_user_req}"
            )

        else:
            # PlantNet failed or low confidence — fall back to Groq Vision
            fail_reason = pn.get("error", "low confidence")
            plant_context = f"[PlantNet unavailable: {fail_reason}. Using vision fallback.]\n\n"
            ...
            groq_prompt = (
                ...
                f"{weather_ctx}\n\n"
                f"{plant_context}"
                f"You are looking at a plant photo sent by a UAE gardener.\n"
                f"Respond as a 20-year master gardener — give your immediate professional verdict.\n\n"
                f"CRITICAL — what NOT to do:\n"
                f"• Do NOT narrate what you see or list observations\n"
                f"• Do NOT state a confidence percentage\n"
                f"• Do NOT number your reasoning steps\n\n"
                f"WHAT TO DO:\n"
                f"• Name the plant directly and with authority\n"
                f"• Give your health verdict in one sentence\n"
                f"• Use Stage 1 format — short, direct, expert\n"
                f"• If healthy, say so warmly. Do not manufacture concerns.\n"
                f"• If two species look very similar, name the most likely one first,\n"
                f"  then ask ONE specific visual question to confirm.\n\n"
                f"User request: {_user_req}"
            )

        # ── 2. Groq generates the UAE care response ───────────────────────────
        GROQ_VISION_MODELS = [
        ...
        for _model in GROQ_VISION_MODELS:
            try:
                ...
                return result_text, engine_used
        ...
        return (
            f"Image analysis failed. Tried: {tried}\n\n"
            "Please try again in a moment or type a description of your plant problem.",
            "error"
        )
