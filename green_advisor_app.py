...
def run_diagnosis(user_message: str, image_b64: str | None, city: str) -> tuple[str, str]:
    ...
    if image_b64 is not None and gemini_client is not None:
        ...
        # Model fallback chain — each model has its own independent free-tier
        # quota pool. If the primary is exhausted (429), the next is tried
        # automatically so the user never hits a dead end on the free tier.
        # Model fallback order — largest free quota first, separate pools.
        # gemini-1.5-flash-8b was REMOVED from v1beta API (causes 404).
        GEMINI_VISION_MODELS = [
            "gemini-2.0-flash-lite",  # 1500 RPD free, 30 RPM — primary
            "gemini-1.5-flash",       # 1500 RPD free, 15 RPM — secondary pool
            "gemini-2.0-flash",       #  200 RPD free, 15 RPM — last resort
        ]
        ...
        for _model_name in GEMINI_VISION_MODELS:
            try:
                ...
                # resp.text returns None when Gemini blocks/filters the response.
                ...
                if result_text is None:
                    ...
                    try:
                        ...
                        pass
                    raise ValueError(
                        f"Gemini ({_model_name}) returned empty/blocked response: {block_reason}"
                    )
                ...
                # Only retry the next model on quota/rate-limit errors
                if ("429" in str(_e) or "quota" in err_lower
                        or "resource_exhausted" in err_lower
                        or "404" in str(_e) or "not_found" in err_lower
                        or "deprecated" in err_lower):
                    continue   # try next model in chain
                else:
                    break      # non-quota error — no point retrying other models

        # All models exhausted or a non-quota error — return formatted error
        err_str = str(last_err) if last_err else "All Gemini vision models failed"
        gemini_error_msg = (
            "Image analysis failed. Gemini error:\n\n" + err_str + "\n\n"
            "What to check:\n"
            ...
            "4. If all 3 free-tier models show 429, your daily quota is exhausted.\n"
            "   Free tier resets at midnight Pacific Time. Or enable billing at\n"
            "   https://ai.dev to get paid-tier quota immediately.\n"
        )
        if user_message.strip():
            gemini_error_msg += (
                "\nYour text: '" + user_message + "'\n"
