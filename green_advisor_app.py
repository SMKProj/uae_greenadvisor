...
if "pending_img_b64"  not in st.session_state: st.session_state.pending_img_b64  = None
    ...
    if image and gemini_model:
        try:
            vision_text = (
                f"{SYSTEM_PROMPT}\n\n"
                f"{weather_ctx}\n\n"
                f"CRITICAL: Identify the plant strictly from what you SEE in the "
                f"image. Examine leaf shape, flower colour, stem, growth pattern. "
                f"Do NOT guess species from user text. Visual analysis takes priority.\n\n"
                f"User request: {user_message}"
            )
            resp = gemini_model.generate_content([image, vision_text])
            st.session_state.last_call_time = time.time()
            return resp.text, weather
        except Exception as e:
            user_message = (
                f"[Vision failed: {str(e)[:80]}. Diagnose from text only.] "
                f"{user_message}"
            )
            enriched = f"{weather_ctx}\n\nUser: {user_message}"
    elif image and not gemini_model:
        user_message = (
            f"[Image uploaded but vision unavailable — GEMINI_API_KEY not set. "
...
with st.sidebar:
    ...
    if uploaded_file:
        ...
        st.session_state.pending_img_b64  = b64_img
        st.image(pil_img, caption=f"📸 {uploaded_file.name}", use_container_width=True)
        st.caption("✅ Image ready — type your message and click Send")
...
if not st.session_state.messages:
    ...
    for msg in st.session_state.messages:
        if msg["role"] == "user":
            ...
            # Render image thumbnail via st.image (avoids HTML tag rendering bug)
            if msg.get("img_b64"):
                img_bytes = base64.b64decode(msg["img_b64"])
                pil_thumb = Image.open(io.BytesIO(img_bytes))
                _, img_col = st.columns([2, 1])
                with img_col:
                    st.image(
                        pil_thumb,
                        caption=f"📸 {msg.get('img_name', 'image')}",
                        use_container_width=True,
                    )
            # Text bubble
            st.markdown(
                f"<div class='msg-wrap-user'><div style='max-width:72%'>"
                f"<div class='bubble-user'>{safe_text}</div></div></div>",
                unsafe_allow_html=True
            )
        else:
            ...
            vision_badge = ""
            if msg.get("used_vision"):
                vision_badge = "<span style='font-size:11px; background:#d8f3dc; color:#1b4332; padding:2px 8px; border-radius:10px; margin-left:6px'>📸 Vision analysis</span>"
            st.markdown(f"""
            <div class='msg-wrap-agent'>
                <div>
                    <div class='bubble-label'>🌿 GreenAdvisor {vision_badge}</div>
...
with col1:
    if st.session_state.pending_image is not None:
        st.caption(f"📸 Image attached: {st.session_state.pending_img_name} — will be analyzed with your message")
...
# ── Handle Send ──────────────────────────────────────────────
if send_clicked and (user_input.strip() or st.session_state.pending_image is not None):
    ...
    # Store user message with optional image data for display
    user_msg = {
        "role":     "user",
        "content":  msg_text,
        "img_b64":  st.session_state.pending_img_b64  if has_image else None,
        "img_name": st.session_state.pending_img_name if has_image else None,
    }
    st.session_state.messages.append(user_msg)

    with st.spinner("🌿 GreenAdvisor is analyzing your plant..."):
        plant_image = st.session_state.pending_image
        reply, _    = diagnose(msg_text, image=plant_image, city=city)

    # Store agent reply with vision flag
    st.session_state.messages.append({
        "role":        "assistant",
        "content":     reply,
        "used_vision": has_image and gemini_model is not None,
    })
