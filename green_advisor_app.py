...
def run_diagnosis(user_message: str, image_b64: str | None, city: str) -> tuple[str, str]:
    ...
    if image_b64 is not None and gemini_client is not None:
        ...
        GEMINI_VISION_MODELS = [
            "gemini-1.5-flash-8b",   # highest free RPD quota (1500/day)
            "gemini-2.0-flash",      # 200 RPD free tier
            "gemini-1.5-flash",      # 1500 RPD free tier (separate pool)
        ]
        ...
        for _model_name in GEMINI_VISION_MODELS:
            try:
                ...
                if "429" in str(_e) or "quota" in err_lower or "resource_exhausted" in err_lower:
