...
def run_diagnosis(user_message: str, image_b64: str | None, city: str) -> tuple[str, str]:
    ...
    if image_b64 is not None and gemini_client is not None:
        ...
        # ── Gemini Vision: model fallback chain ──────────────────────────────
        # Free-tier quota is PER-PROJECT, not per-model — so all models share
        # the same daily token budget. However RPM (per-minute) limits are
        # per-model. Strategy: on RPM-only exhaustion (retryDelay < 90s) wait
        # and retry same model once. On daily exhaustion, try next model.
        # Models tried in order:
        GEMINI_VISION_MODELS = [
            "gemini-2.0-flash-lite",   # try first — lowest RPM pressure
            "gemini-1.5-flash",        # separate model endpoint
        ...
        def _parse_retry_delay(err_str):
            """Extract retryDelay seconds from Gemini 429 error string."""
            m = _re.search(r"retryDelay.*?([0-9]+)s", err_str)
            return int(m.group(1)) if m else None

        last_err      = None
        model_results = []   # track what happened per model for the error report

        for _model_name in GEMINI_VISION_MODELS:
            _tried_once = False
            while True:   # allows one retry after RPM wait
                try:
                    ...
                    if result_text is None:
                        ...
                        try:
                            ...
                            if fr and fr.block_reason:
                                ...
                                block_reason = str(resp.candidates[0].finish_reason)
                        except Exception:
                            pass
                        raise ValueError(
                            f"Empty/blocked response from {_model_name}: {block_reason}"
                        )

                    return result_text, f"gemini-vision ({_model_name})"
                    ...
                    is_rpm_only = (
                        is_quota
                        and "perminute" in err_lower.replace("_","").replace(" ","")
                        and "perday"    not in err_lower.replace("_","").replace(" ","")
                    )
                    is_404 = "404" in err_str or "not_found" in err_lower

                    if is_rpm_only and not _tried_once:
                        # Per-minute limit only — wait the suggested delay and retry once
                        delay = _parse_retry_delay(err_str) or 20
                        delay = min(delay, 30)   # cap at 30s to avoid Streamlit timeout
                        model_results.append(f"{_model_name}: RPM limit, waiting {delay}s")
                        time.sleep(delay)
                        _tried_once = True
                        continue   # retry same model after wait

                    elif is_quota or is_404:
                        # Daily quota exhausted or model not found — skip to next model
                        short_err = "daily quota exhausted" if is_quota else "model not found (404)"
                        model_results.append(f"{_model_name}: {short_err}")
                        break   # break inner while, continue outer for

                    else:
                        # Non-quota error (bad key, network, content blocked) — stop chain
                        model_results.append(f"{_model_name}: {err_str[:80]}")
                        break

                break   # successful — exit while (unreachable if return fired, safety net)

        # All models failed — build a clear, actionable error message
        err_str = str(last_err) if last_err else "All Gemini vision models failed"
        tried   = " | ".join(model_results) if model_results else "none"

        gemini_error_msg = (
            "Image analysis failed. All Gemini vision models were tried.\n\n"
            f"Models tried: {tried}\n\n"
            "What to do:\n"
            "1. Your free-tier daily quota is exhausted. It resets at midnight\n"
            "   Pacific Time (03:00 UTC). Wait and try again tomorrow.\n"
            "2. To get unlimited quota now: enable billing at https://ai.dev\n"
            "   Gemini Vision costs ~$0.075 per 1000 images (pennies for testing).\n"
            "3. Or create a new Google account and get a fresh API key at\n"
            "   https://aistudio.google.com — update GEMINI_API_KEY in Secrets.\n\n"
            f"Full error: {err_str[:300]}\n"
        )
        if user_message.strip():
            gemini_error_msg += (
                "\nYour text message was saved. Remove the image, then Send again\n"
                "for a text-only diagnosis using Groq (no quota limits)."
            )
