...
# ── City selector bar ─────────────────────────────────────────
# Let user correct the IP-detected city with one tap
_city_names = list(UAE_CITIES.keys())
st.markdown(
    f"<div class='ga-city-bar'>📍 Your city: <span>{detected_city}</span>"
    f"&nbsp;&nbsp;Not right? Change:</div>",
    unsafe_allow_html=True,
)
_city_cols = st.columns(len(_city_names))
for _ci, (_cc, _cn) in enumerate(zip(_city_cols, _city_names)):
    with _cc:
        _is_active = _cn == detected_city
        _btn_style = (
            "background:#2d5a1b;color:#edf7e9;" if _is_active
            else "background:#edf7e9;color:#2d5a1b;"
        )
        if st.button(
            _cn,
            key=f"city_{_ci}_{_wc}",
            help=f"Set your city to {_cn}",
        ):
...
# ── File uploader FIRST (above textarea) ─────────────────────
...
# ── Thumbnail preview (shown between uploader and textarea) ───
if st.session_state.pending_image is not None:
    import base64 as _b64
    _thumb_buf = _io.BytesIO()
    _thumb_pil = st.session_state.pending_image.copy()
    _thumb_pil.thumbnail((80, 80))
    if _thumb_pil.mode != "RGB":
        _thumb_pil = _thumb_pil.convert("RGB")
    _thumb_pil.save(_thumb_buf, format="JPEG", quality=70)
    _thumb_b64 = _b64.b64encode(_thumb_buf.getvalue()).decode()
    _img_name  = _html.escape(st.session_state.pending_img_name or "photo.jpg")

    preview_col, rm_col = st.columns([6, 1])
    with preview_col:
        st.markdown(
            f"<div class='ga-img-preview'>"
            f"<img src='data:image/jpeg;base64,{_thumb_b64}' alt='preview'>"
            f"<div><div class='ga-img-preview-name'>📎 {_img_name}</div>"
            f"<div class='ga-img-preview-size'>Ready for analysis</div></div>"
            f"</div>",
            unsafe_allow_html=True,
        )
    with rm_col:
