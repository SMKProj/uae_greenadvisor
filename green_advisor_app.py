...
def diagnose(user_message, image=None, city="Dubai"):
    ...
    if image and gemini_model:
        try:
            ...
            # Vision failed — add note and fall through to Groq text
            user_message = (
                f"[Note: Image was uploaded but vision analysis failed ({str(e)[:60]}). "
                f"Please diagnose based on the text description only.] "
                f"{user_message}"
            )
    elif image and not gemini_model:
        # Gemini not configured — warn and continue with text
        user_message = (
            f"[Note: Image uploaded but vision is unavailable — "
            f"GEMINI_API_KEY not configured. Diagnosing from text only.] "
            f"{user_message}"
        )
        enriched = f"{weather_ctx}\n\nUser: {user_message}"
...
if not st.session_state.messages:
    ...
    for msg in st.session_state.messages:
        if msg["role"] == "user":
            ...
            # Check if this message has an image attached
            img_html = ""
            if msg.get("img_b64"):
                img_html = f"""
                <div style='text-align:right; margin-bottom:6px'>
                    ...
                    <div style='font-size:11px; color:#8aab98; margin-top:3px'>
                        📸 {html_lib.escape(msg.get("img_name","image"))}
                    </div>
                </div>"""
            st.markdown(f"""
