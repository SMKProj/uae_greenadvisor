...
def run_diagnosis(user_message: str, image_b64: str | None, city: str) -> tuple[str, str]:
    ...
    if image_b64 is not None and gemini_client is not None:
        ...
        try:
            ...
            err_str = str(e)
            # FIX: Surface the real Gemini error instead of silent fallback to Groq.
            # Groq has no image, so the old fallback produced the misleading message
            # "I cannot see the image". Now:
            # - Image-only send  → return clear error immediately (Groq can't help)
            # - Image + text     → fall through to Groq for text-only diagnosis
            if user_message.strip():
                user_message = (
                    "[Gemini Vision failed: " + err_str[:120] + ". "
                    "Text-only diagnosis below.]\n\n" + user_message
                )
                # fall through to Groq below
            else:
                return (
                    "Image analysis failed.\n\n"
                    "Gemini Vision error: " + err_str + "\n\n"
                    "Please check:\n"
                    "1. GEMINI_API_KEY is valid and has quota remaining.\n"
                    "2. Image is JPG / PNG / WebP and under 4 MB.\n"
                    "3. If you see a 429 error, wait a moment and retry.\n\n"
                    "Tip: Type a description of your plant problem and Send "
                    "without an image for a text-based diagnosis.",
                    "error"
                )
