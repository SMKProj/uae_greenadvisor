...
# google-genai removed — vision now handled by Groq (Llama-4)
...
def run_diagnosis(user_message: str, image_b64: str | None, city: str) -> tuple[str, str]:
    ...
    # Pass all 4 cities weather so diagnosis is always context-rich
    weather_ctx = "\n".join(
    ...
    if image_b64 is not None:
        GROQ_VISION_MODELS = [
            "meta-llama/llama-4-scout-17b-16e-instruct",    # faster, try first
        ...
        vision_prompt = (
            ...
            f"User request: {user_message if user_message.strip() else 'Please identify this plant and assess its health for UAE conditions.'}"
        )
...
with left_col:
    ...
    # Emirates dropdown removed — all 4 city weather contexts passed automatically
    selected_city = "Sharjah"   # default; all cities shown in weather grid above
    ...
    refresh_clicked = False   # weather refresh moved; keep variable for handler below
    ...
    if st.session_state.chat_history:
        ...
        for msg in st.session_state.chat_history:
            if msg["role"] != "advisor":
                continue   # only show advisor responses in findings box
            text      = msg["text"]
            safe_text = html_lib.escape(text).replace("\n", "<br>")
            history_html += f"""
            <div class='chat-wrap'>
                <div class='chat-label'>🌿 GreenAdvisor</div>
                <div class='chat-msg-advisor'>{safe_text}</div>
            </div>"""
        history_html += "</div>"
        # Copy button — copies all findings text to clipboard
        history_html += """
        <button class='copy-btn' onclick='
            const box = document.querySelector(".result-box");
            const text = box ? box.innerText.replace("Copy", "").trim() : "";
...
with right_col:
    ...
    # Vision status badge — Groq Vision always ready (uses GROQ_API_KEY)
    st.markdown(
        "<div class='vision-badge vision-ok'>✅ Groq Vision (Llama-4) active</div>",
        unsafe_allow_html=True,
    )
    ...
    uploaded_file = st.file_uploader(
        ...
        label_visibility="collapsed",
        key=f"plant_image_{st.session_state.widget_counter}",
    )
    ...
    if st.session_state.pending_image is not None:
        ...
        st.markdown(
            ...
            "Image ready · Groq Vision (Llama-4)</div>",
            unsafe_allow_html=True,
        )
        if st.button("Remove Image ✕", use_container_width=True):
            ...
            # Rotate widget_counter to force file_uploader to re-render as empty
...
if send_clicked:
    ...
    if not msg and img_b64 is None:
        ...
        with st.spinner("🌿 Analysing your plant..."):
            diagnosis, engine = run_diagnosis(msg, img_b64, "Sharjah")

        # Do NOT append user message to chat_history — findings box shows advisor only

        # Engine tag for transparency
        engine_tag = {
            "groq-text" : "",   # no tag needed for plain text
            "error"     : " [Error]",
        }.get(engine, "")   # groq-vision variants show model name already

        full_reply = diagnosis + f"\n\n— GreenAdvisor{engine_tag}"
        st.session_state.chat_history.append({"role": "advisor", "text": full_reply})

        # Clear image + textarea after send (widget_counter rotation)
        st.session_state.pending_image    = None
        st.session_state.pending_img_name = None
        st.session_state.pending_img_b64  = None
        st.session_state.widget_counter  += 1
...
if clear_clicked:
    ...
    st.session_state.pending_img_name = None
    st.session_state.pending_img_b64  = None
    # Rotate counter → forces text_area AND file_uploader to re-render empty
    st.session_state.widget_counter  += 1
    st.rerun()
...
# ██  FOOTER
# ─────────────────────────────────────────────────────────────
vision_status_str = "✅ Groq Vision (Llama-4) active"
st.markdown(f"""
...
<div class='ga-footer-2'>
    Built By: Sundas Khan &nbsp;|&nbsp;
    Tech Stack: Groq (llama-3.3-70b text · llama-4 vision) &middot;
